...
save_additionals <- function(type, current_group, algorithm){
  # create directory if not exists
  dir.create(file.path("../shiny_app/school_markets/results")) 
  ...
  # save network
  selected_list_name <- str_c(ntw_dir, "/selected_list.rds")
  saveRDS(selected_list, selected_list_name)
  sp_network_name <- str_c(ntw_dir, "/sp_network.rds")
  saveRDS(sp_network, sp_network_name)
}
#-----------------------------
# Subgroup stats
#-----------------------------
...
#### Additionals
map_algo <- function(str_algorithm){
  if (str_algorithm=='fg'){
    algo <- 'fast_greedy'
  } else if(str_algorithm=='wt'){
    algo <- 'walktrap'
  } else if(str_algorithm=='le'){
    algo <- 'leading_eigen'
  } else if(str_algorithm=='lp'){
    algo <- 'label_propagation'
  } else if(str_algorithm=='cl'){
    algo <- 'multi_level'
  }
}
